# EDC_shiny
## Description of EDC_shiny dashboard
...
In this approach random walk with restart on gene co-expression networks starting from MIES ,molecular initiating events, (perturbing genes related to each compounds) will be used before fast geneset enrichment analysis (FGSEA). The results of FGSEA will be used as pathway activation scores to train elastic net GLM classifier with compounds tethering known EDC effect label and compounds labeled as negative controls (decoys). After validation of the classifier by means of cross-validation the obtained model can be used to predict EDC probability of unknown compound from a set of primary perturbing known genes.
- The user can select to optimize the pipeline in terms of the number of edges in the network and sorted genes resulting from random walk before FGSEA using pareto solution.
- This approach was tested to predict the EDC probability of 12 K compounds in CTD using a large toxicogenomics data space composed of DrugMatrix, open TG-gates, LINCS and PPI. For this purpose 24 gene co-expression networks werer compiled using transcritpome level of data and the MIEs from CTD were used in this approach to obtain the probabilty scores of the compounds resulting  from all these 24 networks. While the user can use his/her own network, training set (known EDCs and Decoys) and set of MIES, all  knowledge based data for the 24 precompiled networks and 12K componuds are implemented in EDC shiny together with the possibility to compare the results with ToxPi scores (another prediction approach proposed from ToxCast assay endpoints)
...
<img src="https://github.com/vittoriofortino84/EDC_shiny/blob/master/example/figure/toxicogenomics_pipeline3.png" width="600">
...
<img src="https://github.com/vittoriofortino84/EDC_shiny/blob/master/example/figure/toxicogenomics_pipeline4.png" width="600">
...
<img src="https://github.com/vittoriofortino84/EDC_shiny/blob/master/example/figure/toxicogenomics_pipeline5.png" width="600">
...
<img src="https://github.com/vittoriofortino84/EDC_shiny/blob/master/example/figure/pathway_score.png" width="600">
...
<img src="https://github.com/vittoriofortino84/EDC_shiny/blob/master/example/figure/predicted_score.png" width="600">
...
### Toxpi scores Tab
<img src="https://github.com/vittoriofortino84/EDC_shiny/blob/master/example/figure/toxpi_tab.png" width="600">
